...
router.get('/projects', async (req,res) => {const rows=await prisma.project.findMany({where:{enabled:true},orderBy:{displayOrder:'asc'}});res.json(rows.map(x=>{const c=x.content||{};return{...c,...x,client:x.client||c.client||'',demoUrl:x.demoUrl||c.demoUrl||c.link||'',technologies:Array.isArray(x.technologies)?x.technologies:(c.technologies||[]),results:Array.isArray(x.results)?x.results:(c.results||[]),images:Array.isArray(x.images)?x.images:(c.images||(x.imageUrl?[x.imageUrl]:[]))}}))});
...
router.get('/advertisements',async(req,res)=>{const now=new Date();res.json(await prisma.advertisement.findMany({where:{enabled:true,imageUrl:{not:null},AND:[{OR:[{startDate:null},{startDate:{lte:now}}]},{OR:[{endDate:null},{endDate:{gte:now}}]}]},select:{id:true,title:true,description:true,badge:true,imageUrl:true,ctaText:true,ctaLink:true,displayOrder:true,fitMode:true,positionX:true,positionY:true,durationSeconds:true},orderBy:[{displayOrder:'asc'},{createdAt:'desc'}]}))});
