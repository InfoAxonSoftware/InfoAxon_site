...
export const errorDetails=(error)=>{const status=error.response?.status;const message=messageOf(error);return{status,fields:error.response?.data?.errors||{},summary:(status?'HTTP '+status+': ':'')+message}};
